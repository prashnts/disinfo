...
def get_album_art(fragment: str, media_album: str, is_spotify: bool=False):
    ...
    try:
        ...
        img = Image.open(fp)
        # Dithering helps
        frame = Frame(img.resize((80, 80)).resize((25, 25)).convert('RGBA')).trim(3, 3, 3, 3)
        if is_spotify:
...
def composer(fs: FrameState):
    ...
    if art:
        music_elements.insert(0, SlideIn('np.albumart', 0.8, edge='right', easing=ease.linear.linear).mut(art).draw(fs))
