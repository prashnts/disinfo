from PIL import ImageFont

# All Paths are assumed to be from the package root.

# Tamzen
tamzen__rs = ImageFont.truetype('assets/fonts/TamzenForPowerline5x9r.ttf', 9)
tamzen__rm = ImageFont.truetype('assets/fonts/Tamzen7x13r.ttf', 13)

# Pixel Operator.
px_op__r = ImageFont.truetype('assets/fonts/PixelOperator8.ttf', 8)
px_op_mono_8 = ImageFont.truetype('assets/fonts/PixelOperatorMono8.ttf', 8)
px_op__l = ImageFont.truetype('assets/fonts/PixelOperator.ttf', 16)
px_op__xl = ImageFont.truetype('assets/fonts/PixelOperator.ttf', 32)
px_op__xxl = ImageFont.truetype('assets/fonts/PixelOperator.ttf', 48)

cd_icon = ImageFont.truetype('assets/fonts/CD-IconsPC.ttf', 22)

# Scientifica
scientifica__r = ImageFont.truetype('assets/fonts/scientifica.ttf', 11)
scientifica__b = ImageFont.truetype('assets/fonts/scientificaBold.ttf', 11)
scientifica__i = ImageFont.truetype('assets/fonts/scientificaItalic.ttf', 11)
